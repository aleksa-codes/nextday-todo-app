...
export default function AnimatedCircularProgressBar({
  ...
  const currentPercent = reverse
    ? Math.round(((max - value) / (max - min)) * 100)
    : Math.round(((value - min) / (max - min)) * 100);

  return (
    <div
      className={cn('relative flex size-40 items-center justify-center text-2xl font-semibold', className)}
      style={
        {
          '--circle-size': '100px',
    ...
    >
      <svg fill='none' className='absolute inset-0 size-full' strokeWidth='2' viewBox='0 0 100 100'>
